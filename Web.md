...
- **Identificación Pasiva de Infraestructura:**
    - [ ] Revisar fuentes públicas (Shodan, Censys, etc.) si aplica.
...
#### B. Búsqueda y Análisis de Metarchivos y Contenido Estándar
...
#### C. Revisión Inicial del Contenido Web
...
#### D. Recopilación de Información sobre Dominios y Subdominios
...
### II. Análisis de Peticiones y Respuestas Web
...
### III. Pruebas de Gestión de Identidad
...
### V. Pruebas de Gestión de Sesiones
...
- [ ] **Identificación:**
    - [ ] Identificar el servidor backend y el método de parsing utilizado (ej. ASP.NET concatena, PHP/Apache usa el último).
    - [ ] Enviar múltiples parámetros con el mismo nombre y observar cómo se procesan.
    - [ ] `http://example.com/page?param=val1&param=val2`
- [ ] **Explotación:**
    - [ ] Intentar bypassear filtros de validación o WAFs.
    - [ ] Modificar la lógica de la aplicación.
    - [ ] Acceder a funcionalidades no deseadas.
    - [ ] Probar tanto en parámetros GET como en el cuerpo de POST.
...
- [ ] **Explotación Avanzada:**
    ...
    - [ ] **Lectura de Archivos:** `LOAD_FILE()` (MySQL), `UTL_FILE` (Oracle).
...
#### F. Command Injection (Inyección de Comandos del OS)
...
### VII. Vulnerabilidades de Carga de Archivos (File Upload)
...
### VIII. Vulnerabilidades de Inclusión de Archivos (LFI/RFI)
...
#### C. Edge Side Includes (ESI) Injection
...
#### F. XSLT (Extensible Stylesheet Language Transformations) Injection
...
### X. Pruebas de API y Servicios Web

#### A. Descubrimiento e Identificación de APIs
...
#### B. Pruebas de Autenticación y Autorización en APIs
...
#### C. Pruebas de Inyección en APIs
...
#### D. Exposición Excesiva de Datos
...
#### E. Falta de Recursos y Rate Limiting

- [ ] ¿Ausencia de Rate Limiting en endpoints sensibles (login, APIs costosas)? (API4:2023)
- [ ] ¿Se puede causar DoS con peticiones masivas?

#### F. Asignación Masiva (Mass Assignment)

- [ ] ¿Se pueden modificar campos de objetos internos que no deberían ser accesibles (ej. rol de usuario) enviándolos en la petición JSON/XML? (API6:2023)

#### G. Mala Configuración de Seguridad

- [ ] Cabeceras de seguridad faltantes (HSTS, CSP, etc.). (API7:2023)
...
#### H. Gestión Incorrecta de Activos

- [ ] APIs antiguas u obsoletas aún expuestas sin parches. (API9:2023)
- [ ] APIs de prueba/desarrollo expuestas en producción.

#### I. Consumo Inseguro de APIs (SSRF desde la API)

- [ ] Si la API consume otras APIs o URLs, ¿es vulnerable a SSRF? (API10:2023)

#### J. Ataques Específicos de API (basado en tu lista)
...
#### K. Pruebas SOAP (además de XXE e inyecciones generales)
...
### XI. Pruebas Específicas de CMS (Ejemplo: WordPress)

#### A. Estructura y Archivos Clave de WordPress
...
#### B. Enumeración de WordPress (Manual y con WPScan)
